...
function detectLangFromText(q: string): Lang {
  ...
  const hasTrSignal =
    s.match(/[çğıöşü]/) || /\bmerhaba\b|\bateş\b|\böksür\b|\bishal\b|\bkusma\b|\bbebek\b|\byaş\b|\bay\b/.test(s);
...
function detectUrgent(ageMonths: number, text: string) {
  ...
  const redWords = [
    // TR
    'nefes', 'solunum', 'zorluk', 'morarma', 'mavi', 'havale', 'nöbet', 'nobet',
    'bilinç', 'bayıl', 'tepkisiz', 'hırıltı', 'hirilti',
    // EN
    'breath', 'breathing', 'trouble breathing', 'cyanosis', 'blue', 'seizure',
    'unconscious', 'faint', 'unresponsive', 'wheeze', 'wheezing'
  ];
  const hasRed = redWords.some(w => s.includes(w));

  // 38.x desenleri (TR/EN için yeterli)
  const hasFever = /(?:38(\.|,)?\d?)/.test(s) || s.includes('38 derece') || s.includes('38°');
...
function extractKeywords(q: string) {
  ...
  if (base.some(w => ['öksürük','oksuruk','öksürüyor','oksuruyor','hırıltı','hirilti','balgam','cough','wheeze','wheezing'].includes(w))) extras.push('öksürük');
...
async function askGeminiSmart(
  ...
  // Try contextual first, then ultra-short fallback
  try {
  ...
  try {
    ...
    if (question.trim().length < 12) {
      ...
      return NextResponse.json({
        ...
        meta: { source: 'FALLBACK', llmUsed: false, llmError: null, provider: 'rules', matchedFaqs: 0, urgent: false, language: lang }
      }, { status: 400 });
    }
    ...
    if (aiText) {
      ...
      source = 'FAQ';
      // FAQ metni Türkçe/İngilizce karışık olabilir; olduğu gibi gönderiyoruz.
      answer = `🔸 FAQ\n${faqs[0].answer}`;
    } else {
      source = 'FALLBACK';
      answer =
    ...
    return NextResponse.json({
      ...
      candidates: faqs,
      disclaimer: disclaimerFor(lang),
      meta: { source, llmUsed, llmError, provider, matchedFaqs: faqs.length, urgent, language: lang }
    });
  } catch (e: any) {
    return NextResponse.json(
